...
export function createApp(kb: KnowledgeBase): express.Express {
  const app = express();
  ...
  // Authenticate before parsing request bodies. The chat parser is mounted by
  // chatRouter and deliberately has no size limit; all other API/MCP requests
  // reach this bounded parser below it.
  ...
  // Mount before the bounded parser so the chat route can parse arbitrarily long
  // histories without weakening the limit for browse, MCP, or future API routes.
